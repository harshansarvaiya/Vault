...
const localStore = {
  ...
  getItems(username, isDuress = false) {
    const key = `aegis_vault_${username.toLowerCase()}${isDuress ? '_duress' : ''}`;
    const data = localStorage.getItem(key);
    return data ? JSON.parse(data) : [];
  },
  saveItems(username, items, isDuress = false) {
    const key = `aegis_vault_${username.toLowerCase()}${isDuress ? '_duress' : ''}`;
...
async function handleLogin(username, password) {
  try {
    ...
    if (!salts) {
      ...
      if (localUser) {
        salts = {
          ...
          duressSalt: localUser.duressSalt,
        };
      } else {
        throw new Error('Operator identifier not found. If this is your first visit, initialize a new vault.');
      }
    }
    ...
    if (!authData) {
      ...
      const isMaster = localUser.authHash === authHash;
      const isDuress = localUser.duressHash && localUser.duressHash === authHash;

      if (!isMaster && !isDuress) {
      ...
      authData = {
        ...
        isDuress,
        encryptedVek: isDuress ? localUser.duressEncryptedVek : localUser.encryptedVek,
      };
    }
...
async function handleRegister(username, password, duressPin) {
  try {
    ...
    // 4. Send zero-knowledge registration to server or localStore
    let regData = null;
    try {
      if (!isStaticHosting) {
    ...
    // Always store in localStore as offline backup and for static GitHub Pages hosting
    localStore.saveUser({
      username,
      authSalt,
      authHash,
    ...
    if (decoyItems && decoyItems.length > 0) {
      localStore.saveItems(
        username,
        decoyItems.map((d) => ({
          id: `decoy_${Math.random().toString(36).substring(2, 9)}`,
          encrypted_title: d.encryptedTitle,
          category: d.category,
...
async function loadVaultItems() {
  ...
  try {
    ...
    if (!encryptedItems) {
      encryptedItems = localStore.getItems(state.currentUser.username, state.isDuress);
    }
...
async function saveItem(itemData) {
  ...
  try {
    ...
    const localItems = localStore.getItems(state.currentUser.username, state.isDuress);
    ...
    const itemRecord = {
      ...
      is_favorite: itemData.is_favorite ? 1 : 0,
    ...
    localStore.saveItems(state.currentUser.username, localItems, state.isDuress);
...
async function deleteItem(id) {
  ...
  try {
    ...
    // Update localStore
    const localItems = localStore.getItems(state.currentUser.username, state.isDuress).filter((i) => i.id !== id);
    localStore.saveItems(state.currentUser.username, localItems, state.isDuress);
